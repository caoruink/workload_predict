...
class SampleFile(object):
    ...
    def gbdt_pre(self, x_train, y_train, x_test, y_test):
        ...
        gbdt_outfile.write(
            "fit class model at " + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())) + "\n")
        # rng = np.random.RandomState(1)
        clf = GradientBoostingClassifier(n_estimators=1000, learning_rate=0.1, max_depth=15, random_state=1,
    ...
    def rf_pre(self, x_train, y_train, x_test, y_test):
        """
        做分类预测
        :param x_train:
        :param y_train:
        ...
        :return:
        """
        rf_outfile = open(self.__file_route + "/result/RF/res_rf_"
                          + self.__filename.split(".")[0] + ".txt", 'w')
        rf_outfile.write(
            "fit class model at " + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())) + "\n")
        clf = RandomForestClassifier(n_estimators=100, max_depth=10, random_state=1, min_samples_leaf=5,
                                     min_samples_split=2, class_weight="balanced", oob_score=True)

        for i in np.arange(0, self.__num_target):
            clf.fit(x_train, y_train[:, i])
            y_predict = clf.predict(x_test)
            self.rf_result.append(y_predict)
            # prob = clf.predict_proba(x_test)
            # np.savetxt(self.__file_route + "//prob//GBDT//" + self.__filename.split(".")[0] + str(i) +
            #            "_gbdt_pre_prob.csv", prob, delimiter=',', fmt='%.2f')
            print(classification_report(y_test[:, i], y_predict, target_names=['未过载', '过载']))
            print("score:" + str(clf.score(x_test, y_test[:, i])))
            alarm_rate, error_rate = self.cal_error_class(y_test[:, i], y_predict)
            self.rf_error.append([alarm_rate, error_rate])
            rf_outfile.write("alarm_rate:" + str(alarm_rate) + "    error_rate:" + str(error_rate) + "\n")
            rf_outfile.write(
                "end fit class model at " + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())) + "\n")
        rf_outfile.close()
        print("alarm_rate      error_rate:")
    ...
    def predict_class(self):
        ...
        """
        self.separate_XY()
        x_train, x_test, y_train, y_test = train_test_split(self.__sample_x, self.__sample_y, test_size=0.4,
                                                            random_state=1682)
        # 用决策树做
        print("----------------------------decision tree-----------------------------")
        ...
        np.savetxt("实验结果//SVM_error_" + self.__filename, self.svm_error,
                   delimiter=',', fmt='%.6f')

        print("----------------------------Naive Bayes-----------------------------")
        self.nb_pre(x_train, y_train, x_test, y_test)
        np.savetxt("实验结果//NB_error_" + self.__filename, self.nb_error,
                   delimiter=',', fmt='%.6f')

        # print("----------------------------GBDT-----------------------------")
        # self.gbdt_pre(x_train, y_train, x_test, y_test)
        # np.savetxt("实验结果//GBDT_error_" + self.__filename, self.gbdt_error,
        #            delimiter=',', fmt='%.6f')

        print("----------------------------RF-----------------------------")
        self.rf_pre(x_train, y_train, x_test, y_test)
        np.savetxt("实验结果//rf_error_" + self.__filename, self.rf_error,
                   delimiter=',', fmt='%.6f')

        # print("----------------------------vote-------------------------------")
        # self.vote(y_test)
        # np.savetxt("实验结果//vote_error_" + self.__filename, self.vote_error,
        #            delimiter=',', fmt='%.6f')
        return
    ...
    def vote(self, y_test):
        ...
        self.gbdt_result = np.array(self.rf_result).T
